...
col = [ZZ(o) for o in (K2.denominator()*K2.matrix()).list()]
...
for a,b in product(range(p),repeat = 2):
    ...
    print a,b
    try:
        poly = algdep(x+O(p^(prec-1)),2)
        print poly,poly.discriminant().squarefree_part()
        # # poly = algdep(Qp(p,prec)(x+O(p^prec)),1)
        # poly1 = algdep(Qp(p,prec)(ZZ(x._ntl_rep()[0]))+O(p^(prec)),1)
        # #print factor(poly)
        # print "%s,%s:\t(%s) %s"%(a,b,factor(poly1),poly1.discriminant().squarefree_part())
        # poly2 = algdep(Qp(p,prec)(ZZ(x._ntl_rep()[1]))+O(p^(prec)),1)
        # #print factor(poly)
        # print "%s,%s:\t(%s) %s"%(a,b,factor(poly2),poly2.discriminant().squarefree_part())
    except (ValueError,KeyboardInterrupt,PariError):
        continue
