...
# PhiE = Coh.gen(0) # p = 13, D = 2*3
PhiE = Coh.construct_element_from_vector(col) # p= 11, D = 2*3
# PhiE = Coh.gen(1)
...
Jlog = J.log()
Cp = Jlog.parent()
addpart_exp = (Jlog/(nn)).exp()
for a,b in product(range(p),repeat = 2):
    if a == 0 and b == 0:
        continue
    # print a,b
    multpart = Cp.teichmuller(a + Cp.gen()*b)
    J1 = multpart * addpart_exp
    x,y = getcoords(E,J1,prec)
    try:
        poly = algdep(x.trace()+O(p^(prec)),2)
        #print factor(poly)
        print "%s,%s:\t(%s) %s"%(a,b,factor(poly),poly.discriminant().squarefree_part())
        poly = algdep(x.norm()+O(p^(prec)),2)
        #print factor(poly)
        print "%s,%s:\t(%s) %s"%(a,b,factor(poly),poly.discriminant().squarefree_part())
