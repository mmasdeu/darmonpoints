...
class PSModularSymbolElement(ModuleElement):
    ...
    def hecke(self, ell, algorithm="prep", parallelize = False,precomp_data = None,acting_matrices = None,num_iterations = 1,scaling = 1,progress_bar = None):
        r"""
        ...
        if precomp_data is not None:
            return self.__class__(fork(self._map.hecke)(ell, algorithm, parallelize = parallelize,fname = precomp_data,acting_matrices = acting_matrices,num_iterations = num_iterations,scaling = scaling,progress_bar = progress_bar), self.parent(), construct=True)
        else:
            return self.__class__(self._map.hecke(ell, algorithm, parallelize = parallelize,acting_matrices = acting_matrices,num_iterations = num_iterations,scaling = scaling,progress_bar = progress_bar), self.parent(), construct=True)
    ...
    def Tq_eigenvalue(self, q, p=None, M=None, check=True):
        ...
        if check:
            ...
            if p is None or M is None:
                if qhecke != aq * self:
                    raise ValueError("not a scalar multiple (%s vs %s)"%(qhecke._map[g],aq * self._map[g]))
...
class PSModularSymbolElement_symk(PSModularSymbolElement):
    ...
    def lift(self, p=None, M=None, alpha=None, new_base_ring=None, algorithm='stevens', eigensymbol=False, check=True, parallelize = False,precomp_data = None,progress_bar = None):
        r"""
...
###  on M related to the weight.
        ...
        if algorithm == 'stevens':
            if eigensymbol:
                ...
                newM, eisenloss, q, aq = self._find_extraprec(p, M, alpha, check)
                return self._lift_to_OMS_eigen(p, M, new_base_ring, alpha, newM, eisenloss, q, aq, check, parallelize = parallelize,precomp_data = precomp_data,progress_bar = progress_bar)
            else:
    ...
    def _lift_to_OMS_eigen(self, p, M, new_base_ring, ap, newM, eisenloss, q, aq, check, parallelize = False,precomp_data = None, progress_bar = None):
        r"""
...
#        Phi = Phi * p**s
        ...
        t_start = walltime()
        Psi = apinv * Phi.hecke(p, parallelize = parallelize,precomp_data = precomp_data,acting_matrices = acting_matrices,progress_bar = None)
        t_end = walltime(t_start)
        # Estimate the total time to complete
        eta = (t_end * (newM + 1))/(60*60)
        ...
        Phi = Phi.hecke(p, parallelize = parallelize,precomp_data = precomp_data,acting_matrices = acting_matrices,num_iterations = M,scaling = apinv,progress_bar = progress_bar)
        # attempts = M
    ...
    def p_stabilize_and_lift(self, p=None, M=None, alpha=None, ap=None, new_base_ring=None, \
                               ordinary=True, algorithm=None, eigensymbol=False, check=True, parallelize = False,progress_bar = None):
        """
        ...
        return self._lift_to_OMS_eigen(p=p, M=M, new_base_ring=new_base_ring, ap=alpha, newM=newM, eisenloss=eisenloss, q=q, aq=aq, check=check, parallelize = parallelize,progress_bar = progress_bar)
