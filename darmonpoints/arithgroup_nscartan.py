...
class ArithGroup_nscartan(ArithGroup_generic):
    ...
    def __init__(
        ...
        self.Obasis = [
            matrix(ZZ, 2, 2, v)
            for v in [[1, 0, 0, 1], [0, q, 0, 0], [0, N * epsinv, N, 0], [0, 0, 0, q]]
        ]
        ...
        if compute_presentation:
            ...
            self._gens.append(
                self.element_class(
                    self, quaternion_rep=self.Ugens[i], word_rep=[i + 1], check=False
                )
            )
    ...
    def _get_word_rep_initial(self, delta):  # nonsplitcartan
        ...
        try:
            ans = list(
                self._Gamma_farey.word_problem(SL2Z(delta.list()), output="standard")
            )
        except (RuntimeError, AssertionError):
            try:
                ans = list(
                    self._Gamma_farey.word_problem(
    ...
    def element_of_norm(
        ...
        for a, b in product(range(q * M), repeat=2):
            if (a**2 * llinv - b**2 * eps * llinv - 1) % (q * M) == 0 and (
                b * eps
            ) % M == 0:
                verbose("Found a=%s, b=%s" % (a, b))
                found = True
