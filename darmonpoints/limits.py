...
def factorize_matrix(m, M):
    ...
    aabs = ZZ(a).abs()
    Zm = Zmod(M)
    for alphamul in sorted(
        range(floor(-aabs.sqrt(50) / M), floor(aabs.sqrt(50) / M)),
        key=lambda x: ZZ(x).abs(),
    ):
        alpha = 1 + M * alphamul
        if alpha.abs() >= aabs:
            continue
...
def find_lambda(M, p, n_results=1):
    ...
    lambda_set = sorted(
        [
            (ZZ(lmbd0 + delta), ZZ(ap + (lmbd0 + delta) * cp))
            for delta in range(-20, +20)
        ],
        key=lambda x: x[1].abs(),
    )
    res = []
    for lmd, aplc in lambda_set:
        res.extend(
            [
                (lmd * p ** (vala - valc), unit * p ** (valc))
                for unit in is_represented_by_unit(aplc, cp, p)
            ]
        )
...
def _find_initial_embedding_list(v0, M, W, orientation, OD, u):
    ...
    Blist = [M2Z([1, 0, 0, 1])] + [
        M2Z([ZZ(M / d1), i, 0, d1])
        for d1 in ZZ(M).divisors()
        for i in range(-2 * ZZ(QQ(d1 / 2).ceil()), 2 * ZZ(QQ(d1 / 2).ceil()) + 1)
    ]
    for B in Blist:
        W_M = B * W * B**-1
        if all([x.is_integral() for x in W_M.list()]) and ZZ(W_M[1, 0]) % M == 0:
            ...
            if (
                ...
                tau0 = compute_tau0(v0, W_M, wD, return_exact=True)
                assert (
                    find_containing_affinoid(p, v0(tau0)).determinant().valuation(p) % 2
                    == 0
                )
...
def find_tau0_and_gtau(
    v0, M, W, orientation=None, extra_conductor=1, algorithm="guitart_masdeu"
):
    F = v0.domain()
    r = F.gen()
    Cp = v0.codomain()
    ...
    if algorithm == "darmon_pollack":
        ...
        u0vec = wD.coordinates_in_terms_of_powers()(
            u
        )  # Finds a,b such that u = a + b w
        ...
        emblist = [
            emb
            for sgn in [+1, -1]
            for emb in _find_initial_embedding_list(v0, M, WD, orientation, OD, sgn * u)
        ]
        ...
        for tau, gtau, sign in list_embeddings:
            ...
            #    continue
            try:
                V = find_limits(tau, gtau, M, v0, method=2)
            ...
            if opt_evals is None or n_evals < opt_evals:
                opt_evals, opt_tau, opt_gtau, opt_sign, opt_V = (
                    n_evals,
                    tau,
                    gtau,
                    sign,
        ...
        raise ValueError(
            'Algorithm must be either "guitart_masdeu" or "darmon_pollack"'
        )
...
def find_optimal_embeddings(F, use_magma=False, extra_conductor=1, magma=None):
    ...
    if use_magma == True or extra_conductor != 1:
        ...
        for I in [F.ideal(cl.gens()) for cl in F.class_group()]:
            ...
            G.append((a, b, c))
    delta = (
        extra_conductor * F.gen() if D % 4 == 1 else 2 * extra_conductor * F.gen()
    )  # delta = sqrt{discriminant}
    r, s = delta.coordinates_in_terms_of_powers()(w)  # w = r + s*delta
    return [
        Matrix(QQ, 2, 2, [r - s * B, -2 * s * C, 2 * s * A, r + s * B]) for A, B, C in G
    ]  # There's a typo in Darmon-Pollack pg.12, fixed by Juan Restrepo
...
def _find_limits_manin_trick(tau, gtau):
    if gtau[0, 0] == 0:
        ...
        if b == 1:
            ...
            if d < 0:
                a, b, c, d = -a, -b, -c, -d
            return _find_limits_manin_trick(
                (b * tau - a) / (d * tau - c), M2Z([0, -1, 1, 0])
            ) + _find_limits_manin_trick(tau, M2Z([-c, a, -d, b]))
...
def _find_limits_prefactoring(tau, gtau, level, v0):
    ...
    if not all(
        is_in_Gamma_1(mat, level, p, determinant_condition=False) for mat in decomp
    ):
        for mat in decomp:
