...
def compute_lift_sparse_cy(list big_system, list indeps, edge_to_eqs):
    ...
    print('len(pivots) = %s'%len(pivots))
    for r, e01 in pivots.items():
        ans[e01] = indeps[r]
    return ans


def compute_lift_sparse_cy_old(list big_system, edge_to_eqs):
    cdef dict pivots = {}
    cdef int ncols = len(edge_to_eqs)
    cdef int nrows = len(big_system)
    cdef int niters = 0
    ...
    for e01 in edge_to_eqs: # for each column...
        ...
        # Find pivot row
        try:
            rws = [(r, len(big_system[r][0])) for r in edge_to_eqs[e01] if r not in pivots]
            r, _ = min(rws, key=lambda o:o[1])
            # r = next(r for r in edge_to_eqs[e01] if r not in pivots)
        except (StopIteration, ValueError):
            continue
        try:
            tmp = big_system[r][0][e01]
            pivots[r] = e01
        except KeyError:
            continue
        assert tmp in [-1, 1]
        if tmp == -1:
            ...
            rw = big_system[r]
            rw[1] = -rw[1]
            for ky, v in rw[0].items():
                rw[0][ky] = -v
        for i in list(edge_to_eqs[e01]):
            if i == r:
                continue
            try:
                tmp = -big_system[i][0][e01]
            except KeyError:
                continue
            # add  (tmp * r) to (ith row)
            big_system[i][1] += tmp * big_system[r][1]
            for e01p, val in big_system[r][0].items() :
                newval = tmp * val
                if e01p in big_system[i][0] :
                    big_system[i][0][e01p] += newval
                    if big_system[i][0][e01p] == 0:
                        try:
                            edge_to_eqs[e01p].remove(int(i))
                        except ValueError:
                            pass
                        del big_system[i][0][e01p]
                else:
                    big_system[i][0][e01p] = newval
                    edge_to_eqs[e01p].append(int(i)) #add(int(i))
    # return { e01 : big_system[r][1] for r, e01 in pivots.items() }
    ans = { e01 : int(0) for e01 in edge_to_eqs }
    print('len(pivots) = %s'%len(pivots))
    for r, e01 in pivots.items():
        ans[e01] = big_system[r][1]
    return ans
