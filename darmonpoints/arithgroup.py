...
def perturb_point_on_arc(x1, x2, z, r):
    ...
    CC = z.parent()
    rnorm = r * (z.imag()**2)
    ans.append(CC(RR(z.real() + rnorm), RR((r2 - (z.real() + rnorm - center)**2).sqrt())))
    ans.append(CC(RR(z.real() - rnorm), RR((r2 - (z.real() - rnorm - center)**2).sqrt())))
    return ans
...
class ArithGroup_rationalquaternion(ArithGroup_fuchsian_generic):
    def __init__(self,discriminant,level,info_magma = None,grouptype = None,magma = None, compute_presentation = True, **kwargs):
        ...
        if isinstance(discriminant,list) or isinstance(discriminant,tuple):
            ...
            self.discriminant = ZZ(QuaternionAlgebra(discriminant[0],discriminant[1]).discriminant())
        else:
    ...
    # rationalquaternion
    def embed_order(self, p, D,prec,outfile = None,return_all = False, extra_conductor=1, **kwargs):
        r'''
...
class ArithGroup_rationalmatrix(ArithGroup_matrix_generic):
    ...
    # rationalmatrix
    def embed_order(self, p, D,prec,orientation = None, use_magma = True,outfile = None, return_all = False, extra_conductor = 1, **kwargs):
        ...
        W = find_optimal_embeddings(K,use_magma = use_magma, extra_conductor = extra_conductor, magma=self.magma)[0]
...
class ArithGroup_nf_generic(ArithGroup_generic):
    ...
    # nf_quaternion
    def embed_order(self, p, D, prec, outfile = None, return_all = False, extra_conductor = 1, F_to_Qp = None, **kwargs):
        r'''
        '''
        verbose('Computing quadratic embedding to precision %s'%prec)
        F = self.base_ring()
    ...
    @cached_method(key = lambda self, N, use_magma, return_all, radius, max_elements: (self, N, return_all, max_elements))
    def element_of_norm(self,N,use_magma = True,return_all = False,radius = -1,max_elements = -1): # in nf_generic
...
class ArithGroup_nf_kleinian(ArithGroup_nf_generic):
    def _init_geometric_data(self, prec = 800, center = None, timeout = 0, **kwargs):
        ...
        filename = kwargs.get('filename', None)
        if filename is not None:
            filename += '_kleinian_{fielddisc}_{quatalg}_{levelnorm}_{grouptype}_{code}.sobj'.format(fielddisc = -self.F.discriminant(), quatalg = self.B.discriminant().norm(), code=ZZ(hash(self.level)).abs(), levelnorm=self.level.norm(), grouptype=self._grouptype)
            try:
                from sage.misc.persist import load
                geom_dict = load(filename)
                for ky, val in geom_dict.items():
        ...
        verbose(f'grouptype = {grouptype}, prec = {prec}, periodenum = {periodenum}, timeout = {timeout}')
...
class ArithGroup_nf_matrix(ArithGroup_nf_kleinian, ArithGroup_matrix_generic):
    ...
    @cached_method(key = lambda self, N, use_magma, return_all, radius, max_elements : (self, N, return_all))
    def element_of_norm(self,N,use_magma = False,return_all = False, radius = None, max_elements = None): # in nf_matrix
