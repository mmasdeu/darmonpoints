...
class OverconvergentDVCocycle(SageObject):
    def __init__(self, G, phi0, base_ring = None):
        ...
        phi_tw = self.HpMer([phi0.evaluate(G.Gpn(G.wp()**-1 *g.quaternion_rep * G.wp())).left_act_by_matrix(wp) for g in Gpgens])
        if distinguished_open == 'Uinf':
            self.phi1 = phi_tw
            self.phi2 = phi
        else:
    ...
    def improve(self, prec):
        ...
        val = -1
        for i in range(prec):
            update_progress(float(val+1) / prec, 'Iterating Up (%s/%s)'%(val+1, prec))
            tmp = self.HpMer.apply_hecke_operator(tmp, p, hecke_data = Up_data, hecke_reps=Up_reps, as_Up=True)
            phi_list[i % 2] += tmp
...
def construct_conjectural_field(D1, D2, magma, extra_conductor_1 = 1, extra_conductor_2 = 1, base=None, return_units=True):
    if base is None:
        try:
            _ = QQ(D1)
            _ = QQ(D2)
    ...
    x = R.gen()
    F1 = base.extension(x**2 - D1, names='aaa') if base != QQ else QuadraticField(D1, names='aaa')
    F2 = base.extension(x**2 - D2, names='bbb') if base != QQ else QuadraticField(D2, names='aaa')
    F1 = F1.absolute_field(names='aaa')
    F2 = F2.absolute_field(names='bbb')

    magma.eval('_, m := RayClassGroup(%s * MaximalOrder(%s), %s)'%(extra_conductor_1, magma(F1).name(), str(list(range(1,len(F1.real_embeddings())+1)))))
    fF1 = R(sage_eval(magma.eval('ElementToSequence(DefiningPolynomial(AbsoluteField(NumberField(AbelianExtension(m)))))')))
    H1 = NumberField(sage_eval(str(gp.polredabs(fF1)), locals={'x' : x}), names='a')
    a = H1.gen()

    magma.eval('_, m := RayClassGroup(%s * MaximalOrder(%s), %s)'%(extra_conductor_2, magma(F2).name(), str(list(range(1,len(F2.real_embeddings())+1)))))
    fF2 = R(sage_eval(magma.eval('ElementToSequence(DefiningPolynomial(AbsoluteField(NumberField(AbelianExtension(m)))))')))
    H2 = NumberField(sage_eval(str(gp.polredabs(fF2)), locals={'x' : x}), names='b')
    b = H2.gen()
    M = NumberField(sage_eval(str(gp.polredabs(H1.composite_fields(H2)[0].polynomial())), locals={'x' : x}),names='z')
    phi1, phi2 = F1.embeddings(M)[0], F2.embeddings(M)[0]
    if return_units:
        return M, list(set([phi1(o) for o in F1.units()] + [phi2(o) for o in F2.units()]))
    else:
        return M
...
def construct_bigfield(Dlist, magma, base=None, ringclassfield=True):
    ...
    for D in Dlist:
        ...
        if ringclassfield:
            magma.eval('_, m := RayClassGroup(%s * MaximalOrder(%s), %s)'%(1, magma(F1).name(), str(list(range(1,len(F1.real_embeddings())+1)))))
            fF1 = R(sage_eval(magma.eval('ElementToSequence(DefiningPolynomial(AbsoluteField(NumberField(AbelianExtension(m)))))')))
    ...
    H = Hlist[0]
    for H2 in Hlist[1:]:
        H = NumberField(sage_eval(str(gp.polredabs(H.composite_fields(H2)[0].polynomial())), locals={'x' : x}),names='z')
    return H


def darmon_vonk_overconvergent_cocycle(G, D, prec, lift=True, working_prec=None, extra_conductor = 1, outfile=None, **kwargs):
    fwrite('# Initializing cocycle...', outfile)
    phi0 = darmon_vonk_cocycle(G, D, prec, extra_conductor=extra_conductor, outfile=outfile,  **kwargs)
...
def darmon_vonk_point(p, DB, D1, D2, prec, working_prec = None, magma = None, extra_conductor_1 = 1, extra_conductor_2 = 1, recognize_point = 'algdep', parity = 'all', degree_bound = 8, outfile=None, **kwargs):
    ...
    if recognize_point not in ['algdep', 'lindep', 'all', 'none']:
        raise ValueError("recognize_point (= '%s' ) should be one of 'algdep', 'lindep', 'all' or 'none'"%str(recognize_point))
    if parity not in ['+', '-', 'even', 'odd','all']:
        raise ValueError("Parity should be one of '+', '-', 'even', 'odd' or 'all'")
    if outfile == 'log':
        outfile = '%s_%s_%s-%s_%s-%s_%s_%s.log'%(p,DB,D1,extra_conductor_1,D2,extra_conductor_2,prec,datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    fwrite('# Calculation with p = %s and prec = %s (working_prec = %s)'%(p,prec, working_prec),outfile)
    ...
    if Phi is None:
        ...
        while ntries < max_tries:
            try:
                ...
                Phi = darmon_vonk_overconvergent_cocycle(G, D1, prec, lift=False, extra_conductor=extra_conductor_1, outfile=outfile, **kwargs)
                needs_lift = True
                break
    ...
    for D2, extra_conductor_2 in product(D2_list, extra_conductor_2_list):
        if D2 == D1 and extra_conductor_2 == extra_conductor_1:
            fwrite("# skipping pair (%s, %s) because it coincides with cocycle"%(D2, extra_conductor_2), outfile)
            continue
        fwrite('# Initializing cycle...', outfile)
        fwrite('#  D2|conductor = %s|%s'%(D2, extra_conductor_2),outfile)
        try:
            theta0, factor = darmon_vonk_cycle(G, D2, working_prec, extra_conductor=extra_conductor_2, padic_field=K, outfile=outfile, **kwargs)
    ...
    for D2, extra_conductor_2, theta0, factor in cycle_list:
        ...
        fwrite(35 * '#', outfile)
        fwrite('# Integrating (%s|%s) and parity "%s"'%(D2, extra_conductor_2, parity), outfile)
        fwrite(35 * '-', outfile)
        try:
        ...
        for parity in parity_list:
            ...
            try:
                F = p.number_field()
                fwrite('F.<{r}> = NumberField({fmin})'.format(r = F.gen(), fmin = F.polynomial()), outfile)
            except AttributeError:
                F = QQ
            fwrite('K.<{g}> = Qp({p}, {prec}).extension({defpoly})'.format(g = K._names[0], p = K.prime(), prec = K.precision_cap(), defpoly = K.defining_polynomial(exact=True)), outfile)
            ...
            if recognize_point in ['lindep','all']:
                try:
                    ...
                    if M is None:
                        M = construct_conjectural_field(D1, D2, magma, extra_conductor_1, extra_conductor_2, return_units=False)
                    fwrite('# Conjectured field of definition generated by polynomial %s'%M.defining_polynomial(), outfile)
                    if M.degree() > max_degree:
                        fwrite('# Too large for lindep (max_degree = %s), skipping.'%max_degree, outfile)
                    else:
                        units = kwargs.get('allowed_units', None)
                        prime_list = kwargs.get('allowed_primes', prime_list_candidates(D1, D2, extra_conductor_1, extra_conductor_2))
                        fwrite('# Prime list = %s'%prime_list, outfile)
                        ans = recognize_DV_lindep(J, M, prime_list, units=units, outfile=outfile, **kwargs)
                        if ans is not None:
                            recog_data.append(ans)
                except KeyboardInterrupt:
                    'Recognition interrupted...'
            if len(recog_data) == 0:
                ans_list.append((J, None, parity, D1, extra_conductor_1, D2, extra_conductor_2, Ftriv))
            elif len(recog_data) == 1:
                ans_list.append((J, recog_data[0], parity, D1, extra_conductor_1, D2, extra_conductor_2, Ftriv))
            else:
                ans_list.append((J, recog_data, parity, D1, extra_conductor_1, D2, extra_conductor_2, Ftriv))
    return ans_list
...
def admissible_discriminants(p, D, bound, magma=None):
    ...
    try:
        ...
        a, b = D
        u0 = F.units()[0]
        for n0, u in product([1, -1, u0, -u0], sum([F.elements_of_norm(nZ) for nZ in range(2,bound)], [])):
            n = n0 * u
            try:
...
def darmon_vonk_cocycle(G, D, prec, extra_conductor = 1, padic_field = None, hecke_data=None, outfile=None, **kwargs):
    ...
    gamma_tau, tau_p = GG.embed_order(p, D, prec, extra_conductor=extra_conductor, outfile=outfile, F_to_Qp = F_to_Qp, padic_field=padic_field, **kwargs)
    K = tau_p.parent()
    V = Divisors(K)
    ...
    if not all(o.degree() == 0 for o in phi0.values()):
        raise ValueError('Not zero-degree-valued: %s'%(str([o.degree() for o in phi0.values()])))
    return phi0

def darmon_vonk_cycle(G, D, prec, extra_conductor = 1, padic_field = None, hecke_data = None, outfile = None, **kwargs):
    p = G.prime()
    F_to_Qp = G.base_ring_local_embedding(prec)
    scaling = kwargs.get('scaling', 1)
    GG = G.large_group()
    Gp = G.small_group()
    F = G.F
    gamma_zeta, zeta = GG.embed_order(p, D, prec, F_to_Qp = F_to_Qp, padic_field=padic_field, extra_conductor=extra_conductor, outfile=outfile, **kwargs)
    ...
    if (distinguished_open == 'Uinf') == twist: # DEBUG - this is awful
        for gamma, D in theta:
            for _, h in G.get_covering(1):
                hg = Gp(G.reduce_in_amalgam(h * gamma.conjugate_by(G.wp()**-1).quaternion_rep))
...
class DVCocycle(SageObject):
    def __init__(self, G, x0, gamma_tau, tau_p):
        ...
        try:
            F_to_R = trd.parent().real_embeddings(CC.precision())[0] # DEBUG: hard coded first embedding
        except AttributeError:
        ...
        if cc == 0:
            ...
            t0 = intersect_geodesic_arcs(self._tau_inf, self._tau_inf_bar, Infinity, ZZ(65)/101 * self._tau_inf + ZZ(36)/101 * self._tau_inf_bar)
...
def prime_list_candidates(D1, D2, extra_conductor_1=1, extra_conductor_2=1):
    ...
    try:
        ...
        D1D2 = fundamental_discriminant(D1) * fundamental_discriminant(D2) * extra_conductor_1**2 * extra_conductor_2**2
...
def analyze(Jlist):
    plus_hits = [f'{D}{"!" if J != 1 else ""}' for J, ans, sgn, D, _ in Jlist if ans is not None and sgn == '+']
    minus_hits = [f'{D}{"!" if J != 1 else ""}' for J, ans, sgn, D, _ in Jlist if ans is not None and sgn == '-']
    print(plus_hits)
    print(minus_hits)
    mystr = 'None' if len(plus_hits) == 0 else ' '.join(plus_hits)
    print(f'# We recognize the + for: {mystr}')
    mystr = 'None' if len(minus_hits) == 0 else ' '.join(minus_hits)
    print(f'# We recognize the - for: {mystr}')
    return

def get_support(ans, Ftriv):
    if ans is None:
        return ' ? ' # Not recognized
    else:
        try:
            deg = ans[1].degree() # Used algdep?
            try:
                Jalg = ans[1].roots(Ftriv)[0][0]
                return ' - ' # Polynomial has a root over "base" field
            except IndexError:
                Ftmp = NumberField(ans[1],names='j')
                return [p for p, e in ans[1].leading_coefficient().factor()]
        except (AttributeError, IndexError, NotImplementedError):
            Jalg, _, _, clist_ans = ans
            try:
                Jalg0 = Jalg.minpoly().roots(Ftriv)[0][0] # Jalg has a root over "base" field
                return ' - '
            except IndexError: # Element is not trivial
                supp = sorted(list(set([p for e, p in clist_ans if p in ZZ])))
                if len(supp) == 0:
                    return ' 1 ' #  Got a nontrivial unit
                else:
                    return supp
    raise RuntimeError

def make_table_bidim(disc_list, Jlist, sign = 'all'):
    if sign == 'all':
        signs = ['+', '-', 'even', 'odd']
    else:
        signs = [sign]
    D1s_list = [f'{d}{sgn}' for d in disc_list for sgn in signs]
    d1_idx = {D1s : i for i, D1s in enumerate(D1s_list)}
    d2_idx = {f'{D2}': i for i, D2 in enumerate(disc_list)}
    table_matrix = [[''] + [f'{D2}' for D2 in disc_list]]
    table_matrix.extend( [[f'{D1s}' ] + ['' for _ in disc_list] for D1s in D1s_list])

    for Jl in Jlist:
        for J, ans, sgn, D1, ec1, D2, ec2, Ftriv in Jl:
            supp = get_support(ans, Ftriv)
            try:
                table_matrix[1+d1_idx[f'{D1}{sgn}']][1+d2_idx[f'{D2}']] = str(supp)
            except KeyError:
                pass
    return table(table_matrix)
