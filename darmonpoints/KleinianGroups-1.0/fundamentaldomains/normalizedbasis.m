...
intrinsic myIndex(LG :: SeqEnum, InSubgroup :: UserProgram) -> RngIntElt, SeqEnum, SeqEnum
{
  ...
  while not S1new cmpeq []  do
      vprint Kleinian: "len(S1new) = ", #S1new;
      S1next := [];
      for s in S1new do
        for g in LG do
          x := s*g;
  ...
  return #S1, S1, SetToSequence(SequenceToSet(S2));
end intrinsic;
...
NB := NormalizedBoundary(F);
return NB,F,FE,IE,Vol,primes,enumtime,pairingtime,ksgtime,totalvect,totalgpelt,u/(8*factor),Center;
...
if Type(Level) eq RngIntElt then
	...
	vprint Kleinian: "========= ========= Computing for a maximal order first";
		OO := MaximalOrder(O);
		LG,_,_,_,_,primes := NormalizedBasisSage(OO : InitialG := InitialG, NbEnum := NbEnum, PeriodEnum := PeriodEnum, BoundPrimes := BoundPrimes, PairingMethod := PairingMethod, GroupType := GroupType, EnumMethod := EnumMethod, zetas := zetas, Maple := false, zK2 := zK2, Center := Center, pr := pr);
                vprint Kleinian: "@@@@";
		index, Repres, Genes := myIndex(LG, func<x | x in O>);
                vprint Kleinian: "@--@";
		vprintf Kleinian, 2: "========= ========= Subgroup has index %o in larger group, %o generators found\n", index, #Genes;
...
vprint Kleinian: "========== NormalizedBasisSage";
H := B`KlnH;
e := H![1,0,0,0];
MH := Parent(Matrix(2, [H| e, 0, 0, e]));
R := BaseField(H);
