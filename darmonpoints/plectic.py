...
from sage.rings.all import RealField,ComplexField,RR,QuadraticField,PolynomialRing,NumberField,QQ,ZZ,Qp,Zmod, Qq
...
def PlecticGroup(p1, p2, quat_data, level, base = None, grouptype = None, seed = None, magma = None, logfile = None, **kwargs):
    ...
    return PlecticGroup_class(base,p1, p2, discriminant,abtuple = (a,b), level = level,seed = seed, grouptype = grouptype,magma = magma, **kwargs)

class PlecticGroup_class(AlgebraicGroup):
    r'''

    '''
    def __init__(self, base, p0, p1, discriminant, abtuple, level = 1, outfile = None, magma = None, **kwargs):
        self.F = base
        self.GS = ArithGroup(base, discriminant, abtuple, p0 * p1 * level, info_magma = kwargs.pop('info_magma', None), magma = magma, compute_presentation = False, **kwargs)
        Gp0 = ArithGroup(base, discriminant, abtuple, p0 * level, info_magma = self.GS, magma = magma, compute_presentation = False, **kwargs)
        Gp1 = ArithGroup(base, discriminant, abtuple, p1 * level, info_magma = self.GS, magma = magma, compute_presentation = False, **kwargs)
        ...
        kwargs['Gpn'] = Gp0
        kwargs['Gn'] = Gp1
        G0 = BigArithGroup_class(base, p0, discriminant, abtuple=abtuple, level=level, extra_q = p1, outfile=outfile, info_magma=self.GS,  magma=magma, **kwargs)
        if not all([G0.Gpn._is_in_order(g) and G0.Gn._is_in_order(g) for g in self.GS._get_O_basis()]):
            raise RuntimeError("Try again later...")

        kwargs['Gn'] = G0.Gpn
        kwargs['Gpn'] = G0.Gn
        G1 = BigArithGroup_class(base, p1, discriminant, abtuple=abtuple, level=level, extra_q = p0, outfile=outfile, info_magma=self.GS, magma=magma, **kwargs)
        if not all([G1.Gpn._is_in_order(g) and G1.Gn._is_in_order(g) for g in self.GS._get_O_basis()]):
            raise RuntimeError("Try again later...")
    ...
    def _repr_(self):
       ...
       S = { (%s), (%s) }, \
       disc = %s, level = %s'%(self.ideal_p0.gens_reduced()[0],\
    ...
    def construct_edge_reps(self, depth):
        ...
        for g0 in self.GG[0].get_edges_upto(depth):
            n_iters += 1
            update_progress(float(n_iters) / total_iters, 'Constructing edge reps for depth %s'%depth)
    ...
    def get_degeneration(self, vv, f, idx):
        ...
        elist = self.GG[idx].BT.leaving_edges(vv[idx]) if even else self.GG[idx].BT.entering_edges(vv[idx])
...
def get_nu1(G, phi, e0, v1, F = None, Co = 0, Ce = 0, scaling=1):
    ...
    sgn = scaling if even else -scaling
    ans = (sgn * sum(evaluate_HC(G, phi, F, G.get_edge_rep(e0,e1), gi) for gi in glist))[0]
    return ans + Ce if even else ans + Co
...
def evaluate_HC(G, phi, F, gamma, gamma_e, edge_dict=None, scaling=1):
    gamma_e_gamma = (gamma_e[0] * gamma[0], gamma_e[1] * gamma[1], gamma_e[2] * gamma[2])
...
def riemann_sum(G,tau0,tau1, gamma, Phi, depth = 1, progress_bar = True, max_iters = -1, F = None, twist=True, edge_dict=None):
    ...
    cov0 = [(r0, he0, G.GG[0].embed(he0, prec), G.GG[1].embed(he0, prec)) for r0, he0 in G.GG[0].get_covering(depth)]
    cov1 = [(r1, he1, G.GG[0].embed(he1, prec), G.GG[1].embed(he1, prec)) for r1, he1 in G.GG[1].get_covering(depth)]
    ...
    for (r0, he0, he00, he01), (r1, he1, he10, he11) in product(cov0, cov1):
        ...
        if tee[0] == Infinity or tee[1] == Infinity:
            continue
        hte0 = projection(((tau0.parent()(tee[0]) - tau0)/(tau0.parent()(tee[0]) - tau0bar)).log())
        hte1 = projection(((tau1.parent()(tee[1]) - tau1)/(tau1.parent()(tee[1]) - tau1bar)).log())
...
def riemann_sum_parallel(G,tau0, tau1, gammaquatrep, phiE, depth, Fdict, njobs=1, twist=True, max_cpus=1):
    ...
    def riemann_sum_parallel_aux(G, tau0, tau1, gamma, Phi, depth, F, idx, njobs, twist=True, progress_bar=False):
        ...
        for (r0, he0, he00, he01), (r1, he1, he10, he11) in \
            islice(product(cov0, cov1), idx, None, njobs):
            n_iters += 1
            if progress_bar:
    ...
    if njobs == 1:
        ...
        for nfin, (_, val) in enumerate(riemann_sum_parallel_aux(arg_list)):
            update_progress(float(nfin)/njobs, 'finished jobs / total = %s/%s'%(nfin+1, njobs))
            try:
                ans += val
            except TypeError:
...
def compute_plectic_point(G, phiE, Fdict, depth, beta, njobs=1, max_cpus=1, outfile=None):
    gammaquatrep, tau0, tau1 = get_cycle(G, beta, 20) # 20 is the precision, okay for now
