...
def fundamental_integral(self, alpha, i, j):
        ...
        for q,_ in tame_level.factor():
                ...
                order_q = ZZ(order_P).lcm(ZZ(order_Pbar)) ## Multiplicative order of pi_q in (O_F tensor Zp)*
                ...
                main_scale /= (lambda_q - lambda_q**(1-ZZ(order_q)) * pi_q**(order_q * i) * pi_q.conjugate()**(order_q * j))
        ...
        for m in exponents:
                ...
                for k in range(num_primes):
                        scalar_sum *= eigenvalues[k]**(-m[k]) *  prime_unifs[k]**(i*m[k]) * prime_unifs[k].conjugate()**(j*m[k])
                ...
                for beta in F.ideal(level).residues():
                        ...
                        B = G.Gpn.B
                        quaternion = G.Gpn(B([(o[0,0] + o[1,1])/2, (o[0,0] - o[1,1])/2, (-o[0,1] - o[1,0])/2, (-o[0,1] + o[1,0])/2])).quaternion_rep
                        ...
                        tmp = sum(ZZ(j).binomial(r) * phi(beta)**(j-r) * phi(N)**r * distribution_beta.moment(r) for r in range(j+1))
                        # print 'tmp = %s'%tmp
                        new_ans += tmp
...
def basic_integral(self, alpha, i, j):
        ...
        return sum( ZZ(j).binomial(r) * ZZ(j).binomial(s) \
                                      * (-1)**(i+j-r-s)  \
                                      * ZZ(K.teichmuller(a)) ** (i-r) \
                                      * ZZ(K.teichmuller(b)) ** (j-s) \
                                      * self.fundamental_integral(alpha,r,s) for r in range(i) for s in range(j))
...
def basic_integral_one_prime(self, alpha, i, j):
        ...
        distribution_alpha = self.evaluate(matrix(F.ring_of_integers(),2,2,[alpha,D,p,E]))

        ## symbol --> overconvergent class --> distributions --> base ring Zp
        K = self.parent().coefficient_module().base_ring()

        ## return the formula
        return sum(sum(  ZZ(i).binomial(r) * ZZ(j).binomial(s) \
                                 * ((alpha - ZZ(K.teichmuller(a))) ** (i - r)) \
                                 * ((alpha.conjugate - ZZ(K.teichmuller(b)) ** (j-r)) \
                                 * (p ** (r+s)) \
                                 * (-1)**(r+1) * (-1)**(s+1) \
                                 * distribution_alpha.moment((r,s)) for r in range(i + 1)) for s in range(j + 1)) / (aP * aPbar))
...
def get_Lseries_term(self, m, n):
        ...
        ## we haven't computed this yet. Go ahead and compute it
                ...
                if m == 0:
                        ...
                        log_binom_m = log_gamma_binomial(p, gamma, z, m, 2 * M) ## p, top gen, power series gen, target term, twice precision
                        if precision is None:
                                precision = min([j + log_binom_m[j].valuation(p)
                ...
                if n == 0:
                        ...
                        log_binom_n = log_gamma_binomial(p, gamma, z, n, 2 * M) ## p, top gen, power series gen, target term, twice precision
                        if precision is None:
                                precision = min([j + log_binom_n[j].valuation(p)
                                                                 for j in range(M, len(lb))])
