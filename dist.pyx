...
cdef class WeightKAction_long(WeightKAction):
    cpdef _compute_acting_matrix(self, g, _M):
        ...
        pMinv = 1.0 / pMinv
        nmod_poly_init2_precomp(t, pM, pMinv, M)
        nmod_poly_init2_precomp(scale, pM, pMinv, M)
        nmod_poly_init2_precomp(xM, pM, pMinv, M+1)
        nmod_poly_init2_precomp(bdy, pM, pMinv, 2)
        nmod_poly_set_coeff_ui(xM, M, 1)
        nmod_poly_set_coeff_ui(t, 0, a)
        nmod_poly_set_coeff_ui(t, 1, c)
        nmod_poly_newton_invert(scale, t, M)
        nmod_poly_set_coeff_ui(bdy, 0, b)
        nmod_poly_set_coeff_ui(bdy, 1, d)
        nmod_poly_mul_trunc_n(scale, scale, bdy, M) # scale = (b+dy)/(a+cy)
        nmod_poly_powmod(t, t, k, xM) # t = (a+cy)^k
        cdef SimpleMat B = SimpleMat(M)
        for col in range(M):
            for row in range(M):
                B._mat[M*col + row] = nmod_poly_get_coeff_ui(t, row)
            if col < M - 1:
                nmod_poly_mul_trunc_n(t, t, scale, M)
        if self._character is not None:
